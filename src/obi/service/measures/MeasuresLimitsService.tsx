...
import { MeasuresLimitsModel } from "../../models/measures/MeasuresLimitsModel";




// Define the shape of the form errors MeasuresLimits
interface MeasuresLimitsFormErrors {
    id?: string[];
    deleted?: string[];
    created?: string[];
    changed?: string[];
...
interface MeasuresLimitsFormState {
    errors: MeasuresLimitsFormErrors;
}

// Define the props that the PostForm component expects
interface MeasuresLimitsPostFormProps {
...
interface MeasuresLimitsPostFormState {
    errors: {
        id?: string[];
        deleted?: string[];
        created?: string[];
...
export const MeasuresLimitsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/measures/limits/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/measures/limits/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/measures/limits/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new MeasuresLimitsModel();
     ...
     */
    async create(
        formState: MeasuresLimitsFormState,
        formData: FormData | any): Promise<MeasuresLimitsFormState> {
        ...
        const url = process.env.httpPath + '/measures/limits';
        ...
        const dataset: MeasuresLimitsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            MeasuresLimitsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/measures/limits/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/measures/limits/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/measures/limits/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<MeasuresLimitsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/measures/limits/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/measures/limits/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

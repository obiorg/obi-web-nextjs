...
import { UsersEmailVerifiedModel } from "../../models/users/UsersEmailVerifiedModel";




    // Define the shape of the form errors UsersEmailVerified
    interface UsersEmailVerifiedFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface UsersEmailVerifiedFormState {
        errors: UsersEmailVerifiedFormErrors;
    }

    // Define the props that the PostForm component expects
    interface UsersEmailVerifiedPostFormProps {
    ...
    interface UsersEmailVerifiedPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const UsersEmailVerifiedService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/users/emailverified/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/users/emailverified/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/users/emailverified/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new UsersEmailVerifiedModel();
     ...
     */
    async create(
        formState: UsersEmailVerifiedFormState,
        formData: FormData | any): Promise<UsersEmailVerifiedFormState> {
        ...
        const url = process.env.httpPath + '/users/emailverified';
        ...
        const dataset: UsersEmailVerifiedFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            UsersEmailVerifiedService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/users/emailverified/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/users/emailverified/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/users/emailverified/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<UsersEmailVerifiedFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/users/emailverified/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/users/emailverified/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

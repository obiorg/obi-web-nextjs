...
import { AnalysesTypesModel } from "../../models/analyses/AnalysesTypesModel";




    // Define the shape of the form errors AnalysesTypes
    interface AnalysesTypesFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface AnalysesTypesFormState {
        errors: AnalysesTypesFormErrors;
    }

    // Define the props that the PostForm component expects
    interface AnalysesTypesPostFormProps {
    ...
    interface AnalysesTypesPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const AnalysesTypesService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/analyses/types/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/analyses/types/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/analyses/types/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new AnalysesTypesModel();
     ...
     */
    async create(
        formState: AnalysesTypesFormState,
        formData: FormData | any): Promise<AnalysesTypesFormState> {
        ...
        const url = process.env.httpPath + '/analyses/types';
        ...
        const dataset: AnalysesTypesFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            AnalysesTypesService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/analyses/types/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/analyses/types/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/analyses/types/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<AnalysesTypesFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/analyses/types/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/analyses/types/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

...
import { UsersHashingAlgorithmsModel } from "../../models/users/UsersHashingAlgorithmsModel";




    // Define the shape of the form errors UsersHashingAlgorithms
    interface UsersHashingAlgorithmsFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface UsersHashingAlgorithmsFormState {
        errors: UsersHashingAlgorithmsFormErrors;
    }

    // Define the props that the PostForm component expects
    interface UsersHashingAlgorithmsPostFormProps {
    ...
    interface UsersHashingAlgorithmsPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const UsersHashingAlgorithmsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/users/hashingalgorithms/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/users/hashingalgorithms/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/users/hashingalgorithms/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new UsersHashingAlgorithmsModel();
     ...
     */
    async create(
        formState: UsersHashingAlgorithmsFormState,
        formData: FormData | any): Promise<UsersHashingAlgorithmsFormState> {
        ...
        const url = process.env.httpPath + '/users/hashingalgorithms';
        ...
        const dataset: UsersHashingAlgorithmsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            UsersHashingAlgorithmsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/users/hashingalgorithms/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/users/hashingalgorithms/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/users/hashingalgorithms/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<UsersHashingAlgorithmsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/users/hashingalgorithms/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/users/hashingalgorithms/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

...
interface EquipementsDropDownProps {
    value: any;
...
export default function EquipementsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: EquipementsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new EquipementsModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'equipement', order: 1 }, { field: 'name', order: 1 }],
                EquipementsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            EquipementsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].equipement + ' - ' + data[i].name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

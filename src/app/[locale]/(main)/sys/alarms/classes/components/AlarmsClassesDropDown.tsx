...
interface AlarmsClassesDropDownProps {
    value: any;
...
export default function AlarmsClassesDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AlarmsClassesDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AlarmsClassesModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'class', order: 1 }, { field: 'name', order: 1 }], AlarmsClassesService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AlarmsClassesService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].class + ' - ' + data[i].name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

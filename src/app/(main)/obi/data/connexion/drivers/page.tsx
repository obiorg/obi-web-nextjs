...
import { MachDriversService } from '@/src/obi/service/data/connexion/MachDriversService';

const Drivers = () => {



    const bodyTemplateId = (rowData: OBI.mach_drivers) => {
        return <InputNumber
            value={rowData.id} disabled readOnly />
    }

    const bodyTemplateDeleted = (rowData: OBI.mach_drivers) => {
        return (
            <Checkbox inputId={rowData.id + '_deleted'} checked={(rowData.deleted ? true : false)} />
        );
    }

    const bodyTemplateCreated = (rowData: OBI.mach_drivers) => {
    ...
    const bodyTemplateChanged = (rowData: OBI.mach_drivers) => {
        if (rowData.length === 1) {
            return '';
        }
    ...
    const [entities, setEntities] = useState<OBI.mach_drivers[]>([]);
    ...
    const columns: OBI.ColumnMeta[] = [
        ...
        { field: 'designation', header: 'Designation', dataType: 'text', sortable: true, filter: true },
    ...
    const defaultMultiSortMeta: Array<DataTableSortMeta> = MachDriversService.defaultMultiSortMeta();
    const defaultFilters: Array<DataTableFilterMeta> = MachDriversService.defaultFilters();
    ...
    const loadLazyData = () => {
        ...
        loadLazyTimeout = setTimeout(() => {
            ...
            // Get Lazy Data
            MachDriversService.getLazy(lazyEventSet).then((data) => {
                // On Good request process data count
                MachDriversService.getLazyCount(lazyEventSet).then((dataCount) => {

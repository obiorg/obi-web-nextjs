...
import { LocationsSubRegionsModel } from "../../models/localisations/LocactionsSubRegionsModel";




// Define the shape of the form errors LocationsSubRegions
interface LocationsSubRegionsFormErrors {
    id?: string[];
    deleted?: string[];
    created?: string[];
    changed?: string[];
...
interface LocationsSubRegionsFormState {
    errors: LocationsSubRegionsFormErrors;
}

// Define the props that the PostForm component expects
interface LocationsSubRegionsPostFormProps {
...
interface LocationsSubRegionsPostFormState {
    errors: {
        id?: string[];
        deleted?: string[];
        created?: string[];
...
export const LocationsSubRegionsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/localisations/subregions/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/localisations/subregions/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/localisations/subregions/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new LocationsSubRegionsModel();
     ...
     */
    async create(
        formState: LocationsSubRegionsFormState,
        formData: FormData | any): Promise<LocationsSubRegionsFormState> {
        ...
        const url = process.env.httpPath + '/localisations/subregions';
        ...
        const dataset: LocationsSubRegionsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            LocationsSubRegionsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/localisations/subregions/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/localisations/subregions/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/localisations/subregions/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<LocationsSubRegionsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/localisations/subregions/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/localisations/subregions/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

...
interface TagsTablesDropDownProps {
...
export default function TagsTablesDropDown({
    id,
    name,
    title,
    value,
    ...
    emptyMessage = 'vide !',
    render = true,
}: TagsTablesDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new TagsTablesModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'table', order: 1 }],
                TagsTablesService.defaultFilters()));
    ...
    useEffect(() => {
        ...
        //     // Get Lazy Data
        //     TagsTablesService.getLazy(lazyEventSet).then((data: any) => {
        ...
        loadLazyTimeout = setTimeout(() => {
            ...
            TagsTablesService.getLazy(lazyEventSet).then((data: any) => {
                ...
                for (let i = lazyParams.first; (i < lazyParams.rows && i < data.length); i++) {
                    ...
                    _catalogs[i] = {
                        label: data[i].table + ' - ' + data[i].designation + ' [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

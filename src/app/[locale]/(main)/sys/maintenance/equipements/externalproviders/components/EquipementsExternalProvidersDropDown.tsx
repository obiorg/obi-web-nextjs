...
interface EquipementsExternalProvidersDropDownProps {
    value: any;
...
export default function EquipementsExternalProvidersDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: EquipementsExternalProvidersDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new EquipementsExternalProvidersModel().
        getStandardParam([{ field: 'company', order: 1 },{ field: 'provider', order: 1 }],
                EquipementsExternalProvidersService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            EquipementsExternalProvidersService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].provider + ' - ' + data[i].name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

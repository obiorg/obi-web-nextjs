...
export default function PostForm({ formAction, type, initialData }: OBI.LocationsPostFormProps) {
    ...
    const onChangedCountry = (e: any) => {
        ...
        if (initialData?.country !== undefined) {
            setCountryOn(true);
            LocationsStateService.count().then((count: any) => {
                setLazyParamsStates(
                    () => {
                        return {
                            ...lazyParamsStates,
    ...
    useEffect(() => {
        // Get full data list
        LocationsCountriesService.list().then((data: any) => {
            if (data.status) {
    ...
    const defaultFiltersStates: Array<DataTableFilterMeta> = LocationsStateService.defaultFilters();
    ...
    useEffect(() => {
        ...
        // Get full data list
        LocationsStateService.getLazy(lazyEventSet).then((data: any) => {

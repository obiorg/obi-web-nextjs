...
interface MeasuresComparatorsDropDownProps {
    value: any;
...
export default function MeasuresComparatorsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: MeasuresComparatorsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new MeasuresComparatorsModel().
            getStandardParam([{ field: 'symbol', order: 1 }, { field: 'name', order: 1 }],
                MeasuresComparatorsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            MeasuresComparatorsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].symbol + ' - ' + data[i].name + ' [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }
    ...
    return <>
        ...
        <Dropdown
            ...
            placeholder="Mesures comparateurs..."
            showClear
            filter

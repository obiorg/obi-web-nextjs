...
interface AnalysesAllowedDropDownProps {
    value: any;
...
export default function AnalysesAllowedDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AnalysesAllowedDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AnalysesAllowedModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'point', order: 1 }, { field: 'type', order: 1 }, { field: 'designation', order: 1 }], AnalysesAllowedService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AnalysesAllowedService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i]?.designation + ' - ' + data[i].analyse_points?.designation + '/' + data[i].analyse_types?.designation + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }
    ...
    return <>
        ...
        <Dropdown
            ...
            placeholder="Analyses autorisées..."
            showClear
            filter

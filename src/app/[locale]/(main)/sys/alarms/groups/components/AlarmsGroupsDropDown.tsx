...
interface AlarmsGroupsDropDownProps {
    value: any;
...
export default function AlarmsGroupsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AlarmsGroupsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AlarmsGroupsModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'group', order: 1 }], AlarmsGroupsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AlarmsGroupsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].group + ' - ' + data[i].comment + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

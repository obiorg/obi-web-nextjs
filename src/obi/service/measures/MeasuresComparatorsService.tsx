...
import { MeasuresComparatorsModel } from "../../models/measures/MeasuresComparatorsModel";




// Define the shape of the form errors MeasuresComparators
interface MeasuresComparatorsFormErrors {
    id?: string[];
    deleted?: string[];
    created?: string[];
    changed?: string[];
...
interface MeasuresComparatorsFormState {
    errors: MeasuresComparatorsFormErrors;
}

// Define the props that the PostForm component expects
interface MeasuresComparatorsPostFormProps {
...
interface MeasuresComparatorsPostFormState {
    errors: {
        id?: string[];
        deleted?: string[];
        created?: string[];
...
export const MeasuresComparatorsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/measures/comparators/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/measures/comparators/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/measures/comparators/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new MeasuresComparatorsModel();
     ...
     */
    async create(
        formState: MeasuresComparatorsFormState,
        formData: FormData | any): Promise<MeasuresComparatorsFormState> {
        ...
        const url = process.env.httpPath + '/measures/comparators';
        ...
        const dataset: MeasuresComparatorsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            MeasuresComparatorsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/measures/comparators/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/measures/comparators/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/measures/comparators/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<MeasuresComparatorsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/measures/comparators/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/measures/comparators/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

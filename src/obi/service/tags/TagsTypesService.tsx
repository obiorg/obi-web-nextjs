...
import { TagsTypesModel } from "../../models/tags/TagsTypesModel";




    // Define the shape of the form errors TagsTypes
    interface TagsTypesFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface TagsTypesFormState {
        errors: TagsTypesFormErrors;
    }

    // Define the props that the PostForm component expects
    interface TagsTypesPostFormProps {
    ...
    interface TagsTypesPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const TagsTypesService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/tags/types/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/tags/types/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/tags/types/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new TagsTypesModel();
     ...
     */
    async create(
        formState: TagsTypesFormState,
        formData: FormData | any): Promise<TagsTypesFormState> {
        ...
        const url = process.env.httpPath + '/tags/types';
        ...
        const dataset: TagsTypesFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            TagsTypesService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/tags/types/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/tags/types/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/tags/types/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<TagsTypesFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/tags/types/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/tags/types/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

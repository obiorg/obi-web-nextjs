...
interface AlarmsRendersDropDownProps {
    value: any;
...
export default function AlarmsRendersDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AlarmsRendersDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AlarmsRendersModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'render', order: 1 }, { field: 'name', order: 1 }], AlarmsRendersService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AlarmsRendersService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].render + ' - ' + data[i].name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

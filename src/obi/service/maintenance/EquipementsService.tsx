...
import { EquipementsModel } from "../../models/maintenance/EquipementsModel";




// Define the shape of the form errors Equipements
interface EquipementsFormErrors {
    id?: string[];
    deleted?: string[];
    created?: string[];
    changed?: string[];
...
interface EquipementsFormState {
    errors: EquipementsFormErrors;
}

// Define the props that the PostForm component expects
interface EquipementsPostFormProps {
...
interface EquipementsPostFormState {
    errors: {
        id?: string[];
        deleted?: string[];
        created?: string[];
...
export const EquipementsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/maintenance/equipements/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/maintenance/equipements/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/maintenance/equipements/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new EquipementsModel();
     ...
     */
    async create(
        formState: EquipementsFormState,
        formData: FormData | any): Promise<EquipementsFormState> {
        ...
        const dataset: EquipementsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            EquipementsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/maintenance/equipements/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/maintenance/equipements/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/maintenance/equipements/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<EquipementsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/maintenance/equipements/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/maintenance/equipements/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

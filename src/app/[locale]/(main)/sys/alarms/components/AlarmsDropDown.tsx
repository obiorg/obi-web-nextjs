...
export default function AlarmsDropDown({
    value,
    onChanged,
    ...
    const [lazyParams, setLazyParams] = useState(
        new AlarmsModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'alarm', order: 1 }, { field: 'name', order: 1 }], AlarmsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AlarmsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].alarm + ' - ' + data[i].name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

...
import { AlarmsGroupsModel } from "../../models/alarms/AlarmsGroupsModel";




    // Define the shape of the form errors AlarmsGroups
    interface AlarmsGroupsFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface AlarmsGroupsFormState {
        errors: AlarmsGroupsFormErrors;
    }

    // Define the props that the PostForm component expects
    interface AlarmsGroupsPostFormProps {
    ...
    interface AlarmsGroupsPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const AlarmsGroupsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/alarms/groups/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/alarms/groups/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/alarms/groups/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new AlarmsGroupsModel();
     ...
     */
    async create(
        formState: AlarmsGroupsFormState,
        formData: FormData|any): Promise<AlarmsGroupsFormState> {
        ...
        const url = process.env.httpPath + '/alarms/groups';
        ...
        const dataset: AlarmsGroupsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            AlarmsGroupsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/alarms/groups/create';
    ...
    async update(
        formState: AlarmsGroupsFormState,
        formData: FormData): Promise<AlarmsGroupsFormState> {
        ...
        const url = process.env.httpPath + '/alarms/groups/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/alarms/groups/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<AlarmsGroupsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/alarms/groups/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/alarms/groups/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

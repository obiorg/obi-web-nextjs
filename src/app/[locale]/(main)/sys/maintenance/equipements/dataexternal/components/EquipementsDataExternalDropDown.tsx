...
interface EquipementsDataExternalDropDownProps {
    value: any;
...
export default function EquipementsDataExternalDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: EquipementsDataExternalDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new EquipementsDataExternalModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'equipement', order: 1 }, { field: 'provider', order: 1 }],
                EquipementsDataExternalService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            EquipementsDataExternalService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].equipements?.equipement + ',' + data[i].equipements?.name + ' - ' + data[i].provider?.name + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

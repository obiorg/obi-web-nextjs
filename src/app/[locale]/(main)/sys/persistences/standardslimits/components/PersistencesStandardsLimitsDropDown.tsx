...
interface PersistencesStandardsLimitsDropDownProps {
    value: any;
...
export default function PersistencesStandardsLimitsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: PersistencesStandardsLimitsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new PersistencesStandardsLimitsModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'tag', order: 1 }, { field: 'pers_standard', order: 1 }, { field: 'name', order: -1 }],
                PersistencesStandardsLimitsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            PersistencesStandardsLimitsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].name + ' - ' + data[i].value + '(' + data[i].tag?.name + '/' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }
    ...
    return <>
        ...
        <Dropdown
            ...
            placeholder="Persistences Standard limites..."
            showClear
            filter

...
import { UsersAccountRoleModel } from "../../models/users/UsersAccountRoleModel";




    // Define the shape of the form errors UsersAccountRole
    interface UsersAccountRoleFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface UsersAccountRoleFormState {
        errors: UsersAccountRoleFormErrors;
    }

    // Define the props that the PostForm component expects
    interface UsersAccountRolePostFormProps {
    ...
    interface UsersAccountRolePostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const UsersAccountRoleService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/users/accountrole/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/users/accountrole/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/users/accountrole/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new UsersAccountRoleModel();
     ...
     */
    async create(
        formState: UsersAccountRoleFormState,
        formData: FormData | any): Promise<UsersAccountRoleFormState> {
        ...
        const url = process.env.httpPath + '/users/accountrole';
        ...
        const dataset: UsersAccountRoleFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            UsersAccountRoleService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/users/accountrole/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/users/accountrole/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/users/accountrole/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<UsersAccountRoleFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/users/accountrole/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/users/accountrole/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

...
import { MeasuresUnitsModel } from "../../models/measures/MeasuresUnitsModel";




// Define the shape of the form errors MeasuresUnits
interface MeasuresUnitsFormErrors {
    id?: string[];
    deleted?: string[];
    created?: string[];
    changed?: string[];
...
interface MeasuresUnitsFormState {
    errors: MeasuresUnitsFormErrors;
}

// Define the props that the PostForm component expects
interface MeasuresUnitsPostFormProps {
...
interface MeasuresUnitsPostFormState {
    errors: {
        id?: string[];
        deleted?: string[];
        created?: string[];
...
export const MeasuresUnitsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/measures/units/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/measures/units/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/measures/units/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new MeasuresUnitsModel();
     ...
     */
    async create(
        formState: MeasuresUnitsFormState,
        formData: FormData | any): Promise<MeasuresUnitsFormState> {
        ...
        const url = process.env.httpPath + '/measures/unitis';
        ...
        const dataset: MeasuresUnitsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            MeasuresUnitsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/measures/units/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/measures/units/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/measures/units/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<MeasuresUnitsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/measures/units/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/measures/units/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

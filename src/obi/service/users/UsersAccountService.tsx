...
import { UsersAccountModel } from "../../models/users/UsersAccountModel";




    // Define the shape of the form errors UsersAccount
    interface UsersAccountFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface UsersAccountFormState {
        errors: UsersAccountFormErrors;
    }

    // Define the props that the PostForm component expects
    interface UsersAccountPostFormProps {
    ...
    interface UsersAccountPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const UsersAccountService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/users/account/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/users/account/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/users/account/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new UsersAccountModel();
     ...
     */
    async create(
        formState: UsersAccountFormState,
        formData: FormData | any): Promise<UsersAccountFormState> {
        ...
        const url = process.env.httpPath + '/users/account';
        ...
        const dataset: UsersAccountFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            UsersAccountService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/users/account/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/users/account/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/users/account/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<UsersAccountFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/users/account/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/users/account/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

...
import { AlarmsClassesModel } from "../../models/alarms/AlarmsClassesModel";




    // Define the shape of the form errors AlarmsClasses
    interface AlarmsClassesFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface AlarmsClassesFormState {
        errors: AlarmsClassesFormErrors;
    }

    // Define the props that the PostForm component expects
    interface AlarmsClassesPostFormProps {
    ...
    interface AlarmsClassesPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const AlarmsClassesService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/alarms/classes/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/alarms/classes/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/alarms/classes/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new AlarmsClassesModel();
     ...
     */
    async create(
        formState: AlarmsClassesFormState,
        formData: FormData|any): Promise<AlarmsClassesFormState> {
        ...
        const url = process.env.httpPath + '/alarms/classes';
        ...
        const dataset: AlarmsClassesFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            AlarmsClassesService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/alarms/classes/create';
    ...
    async update(
        formState: AlarmsClassesFormState,
        formData: FormData): Promise<AlarmsClassesFormState> {
        ...
        const url = process.env.httpPath + '/alarms/classes/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/alarms/classes/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<AlarmsClassesFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/alarms/classes/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/alarms/classes/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

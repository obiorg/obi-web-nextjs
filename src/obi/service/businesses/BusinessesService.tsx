...
import { BusinessesModel } from "../../models/businesses/BusinessesModel";
    ...
    // Define the shape of the form errors businesses
    interface BusinessesFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface BusinessesFormState {
        errors: BusinessesFormErrors;
    }

    // Define the props that the PostForm component expects
    interface BusinessesPostFormProps {
    ...
    interface BusinessesPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const BusinessesService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/businesses/lazy/' + lazy.lazyEvent;
        ...
        try {
            ...
            if (res.ok) {
                ...
                const dataset: OBI.businesses[] = await res.json();
                return dataset;
            } else {
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/businesses/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
                const dataset: OBI.businesses = val;
                return val;
            } else {
                if (res.status === 404) throw new Error('404, Not found');
                if (res.status === 500) throw new Error('500, internal server error');
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/businesses/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
     ...
     */
    async create(
        formState: BusinessesFormState,
        formData: FormData | any): Promise<BusinessesFormState> {
        ...
        const url = process.env.httpPath + '/businesses';
        ...
        // console.log("BusinessesService response", res);
        const dataset: BusinessesFormState = await res.json();
        // console.log('BusinessesService >> result from api Businesses ', dataset);
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            BusinessesService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/businesses/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/businesses/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/businesses/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<BusinessesFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/businesses/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<OBI.businesses[]> {
        const url = process.env.httpPath + '/businesses/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: OBI.businesses[] = await res.json();
        // console.log("Businesses dataset", dataset);
        return dataset;
    },

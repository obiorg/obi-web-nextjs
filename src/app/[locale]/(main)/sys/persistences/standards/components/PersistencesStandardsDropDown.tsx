...
interface PersistencesStandardsDropDownProps {
    value: any;
...
export default function PersistencesStandardsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: PersistencesStandardsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new PersistencesStandardsModel().
            getStandardParam([{ field: 'company', order: 1 }, { field: 'tag', order: 1 }, { field: 'created', order: -1 }],
                PersistencesStandardsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            PersistencesStandardsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].tag?.name + ' - ' + data[i].vFloat + '(' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }
    ...
    return <>
        ...
        <Dropdown
            ...
            placeholder="Persistences Standard..."
            showClear
            filter

...
interface MeasuresLimitsGroupsDropDownProps {
    value: any;
...
export default function MeasuresLimitsGroupsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: MeasuresLimitsGroupsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new MeasuresLimitsGroupsModel().
            getStandardParam([{ field: 'business', order: 1 }, { field: 'company', order: 1 }, { field: 'group', order: 1 }, { field: 'designation', order: 1 }],
                MeasuresLimitsGroupsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            MeasuresLimitsGroupsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].group + ' - ' + data[i].designation + '(' + data[i].business + '/' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }
    ...
    return <>
        ...
        <Dropdown
            ...
            placeholder="Mesure - Limites Groupes..."
            showClear
            filter

...
const Machines = () => {
    ...
    const bodyTemplateId = (rowData: OBI.machines) => {
        return <InputNumber
            value={rowData.id} disabled readOnly />
    }

    const bodyTemplateDeleted = (rowData: OBI.machines) => {
        return (
            <Checkbox inputId={rowData.id + '_deleted'} checked={(rowData.deleted ? true : false)} />
        );
    }

    const bodyTemplateCreated = (rowData: OBI.machines) => {
    ...
    const bodyTemplateChanged = (rowData: OBI.machines) => {
        if (rowData.length === 1) {
            return '';
        }
    ...
    const bodyTemplateMQTT = (rowData: OBI.machines) => {
        return (
            <Checkbox inputId={rowData.id + '_mqtt'} checked={(rowData?.mqtt ? true : false)} />
        );
    }

    const bodyTemplateMQTTPassword = (rowData: OBI.machines) => {
        return <Password value={rowData.mqtt_passwords} disabled />
    }

    const bodyTemplateWebHook = (rowData: OBI.machines) => {
        return (
            <Checkbox inputId={rowData.id + '_webHook'} checked={(rowData.webhook ? true : false)} />
        );
    }

    const bodyTemplateWebHookSecret = (rowData: OBI.machines) => {
        return <Password value={rowData?.webhook_secret} disabled />
    }
    ...
    const [entities, setEntities] = useState<OBI.machines[]>([]);
    ...
    const columns: OBI.ColumnMeta[] = [
        ...
        { field: 'port', header: 'Port', dataType: 'numeric', sortable: true, filter: true },
        { field: 'name', header: 'Nom', dataType: 'text', sortable: true, filter: true },
        { field: 'rack', header: 'Rack', dataType: 'numeric', sortable: true, filter: true },
        { field: 'slot', header: 'Slot', dataType: 'numeric', sortable: true, filter: true },
        { field: 'driver', header: 'Driver', dataType: 'numeric', sortable: true, filter: true },
        { field: 'mqtt', header: 'MQTT ON', dataType: 'numeric', bodyTemplate: bodyTemplateMQTT, sortable: true, filter: true },
        { field: 'mqtt_user', header: 'MQTT Utilisateur', dataType: 'text', sortable: true, filter: true },
        { field: 'mqtt_password', header: 'MQTT Password', dataType: 'text', bodyTemplate: bodyTemplateMQTTPassword, sortable: true, filter: true },
        { field: 'webhook', header: 'Webhook ON', dataType: 'numeric', bodyTemplate: bodyTemplateWebHook, sortable: true, filter: true },
        { field: 'webhook_secret', header: 'Webhook Secret', dataType: 'text', bodyTemplate: bodyTemplateWebHookSecret, sortable: true, filter: true },
        { field: 'bus', header: 'Bus', dataType: 'numeric', sortable: true, filter: true },

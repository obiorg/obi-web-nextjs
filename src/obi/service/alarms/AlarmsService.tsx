...
import { AlarmsModel } from "../../models/alarms/AlarmsModel";




    // Define the shape of the form errors Alarms
    interface AlarmsFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface AlarmsFormState {
        errors: AlarmsFormErrors;
    }

    // Define the props that the PostForm component expects
    interface AlarmsPostFormProps {
...
export const AlarmsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/alarms/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/alarms/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/alarms/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
     ...
     */
    async create(
        formState: AlarmsFormState,
        formData: FormData|any): Promise<AlarmsFormState> {
        ...
        const url = process.env.httpPath + '/alarms';
        ...
        const dataset: AlarmsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            AlarmsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/alarms/create';
    ...
    async update(
        formState: AlarmsFormState,
        formData: FormData): Promise<AlarmsFormState> {
        ...
        const url = process.env.httpPath + '/alarms/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/alarms/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<AlarmsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/alarms/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/alarms/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

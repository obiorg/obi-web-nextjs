...
interface AnalysesMethodsDropDownProps {
    value: any;
...
export default function AnalysesMethodsDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AnalysesMethodsDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AnalysesMethodsModel().
            getStandardParam([{ field: 'business', order: 1 }, { field: 'company', order: 1 }, { field: 'method', order: 1 }, { field: 'designation', order: 1 }], AnalysesMethodsService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AnalysesMethodsService.getLazy(lazyEventSet).then((data: any) => {
                for (let i = lazyParams.first; i < lazyParams.rows; i++) {
                    _catalogs[i] = {
                        label: data[i].method + ' - ' + data[i].designation + '(' + data[i].business + '/' + data[i].company + ') [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

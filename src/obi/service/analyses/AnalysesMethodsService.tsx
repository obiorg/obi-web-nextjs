...
import { AnalysesMethodsModel } from "../../models/analyses/AnalysesMethodsModel";




    // Define the shape of the form errors AnalysesMethods
    interface AnalysesMethodsFormErrors {
        id?: string[];
        deleted?: string[];
        created?: string[];
        changed?: string[];
    ...
    interface AnalysesMethodsFormState {
        errors: AnalysesMethodsFormErrors;
    }

    // Define the props that the PostForm component expects
    interface AnalysesMethodsPostFormProps {
    ...
    interface AnalysesMethodsPostFormState {
        errors: {
            id?: string[];
            deleted?: string[];
            created?: string[];
...
export const AnalysesMethodsService = {
    ...
    async getLazy(lazy: any): Promise<any> {
        const url = process.env.httpPath + '/analyses/methods/lazy/' + lazy.lazyEvent;
    ...
    async getLazyCount(lazy: any) {
        const url = process.env.httpPath + '/analyses/methods/lazy/count/' + lazy.lazyEvent;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
    ...
    async getById(id: any) {
        const url = process.env.httpPath + '/analyses/methods/' + id;
        try {
            const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
            if (res.ok) {
                const val = await res.json();
    ...
    defaultMultiSortMeta(): any {
        const model = new AnalysesMethodsModel();
     ...
     */
    async create(
        formState: AnalysesMethodsFormState,
        formData: FormData | any): Promise<AnalysesMethodsFormState> {
        ...
        const url = process.env.httpPath + '/analyses/methods';
        ...
        const dataset: AnalysesMethodsFormState = await res.json();
        return dataset;
    ...
    async processAll(formState: any, datas: any): Promise<any> {
        ...
        datas.forEach((row:any, index:any) => {
            AnalysesMethodsService.create(formState, row).then((res_row) => {
    ...
    async createMany(data: any[]): Promise<any[]> {
        ...
        const url = process.env.httpPath + '/analyses/methods/create';
    ...
    async update(
        ...
        const url = process.env.httpPath + '/analyses/methods/' + data.id;
    ...
    async updateMany(
        ...
        const url = process.env.httpPath + '/analyses/methods/update';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async delete(id: any): Promise<AnalysesMethodsFormState> {
    ...
    async deleteMany(
        ...
        const url = process.env.httpPath + '/analyses/methods/delete';
        // console.log(data);
        const res = await fetch(
            url,
            {
    ...
    async download(lazy: any): Promise<any[]> {
        const url = process.env.httpPath + '/analyses/methods/download/' + lazy.lazyEvent;
        const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache' } })
        const dataset: any[] = await res.json();
        return dataset;
    },

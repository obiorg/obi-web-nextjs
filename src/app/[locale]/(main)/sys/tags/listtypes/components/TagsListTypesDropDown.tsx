...
import { TagsListTypeModel } from "@/src/obi/models/tags/TagsListTypeModel"
import { TagsListTypeService } from "@/src/obi/service/tags/TagsListTypeService"
...
interface TagsListTypesDropDownProps {
...
export default function TagsListTypesDropDown({
    id,
    name,
    title,
    value,
    ...
    emptyMessage = 'vide !',
    render = true,
}: TagsListTypesDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new TagsListTypeModel().
            getStandardParam([{ field: 'designation', order: 1 }],
                TagsListTypeService.defaultFilters()));
    ...
    const loadData = () => {
        ...
        loadLazyTimeout = setTimeout(() => {
            ...
            // Get Lazy Data
            TagsListTypeService.getLazy(lazyEventSet).then((data: any) => {
                ...
                for (let i = lazyParams.first; (i < lazyParams.rows && i < data.length); i++) {
                    ...
                    _catalogs[i] = {
                        label: data[i].designation + ' - ' + data[i].comment + ' [' + data[i].id + ']',
                        value: data[i].id,
                        catalogs: data[i]
                    };
                }

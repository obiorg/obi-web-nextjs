...
interface AnalysesTypesDropDownProps {
    value: any;
...
export default function AnalysesTypesDropDown({
    value,
    onChanged,
    placeholder, tooltip, tooltipOptions
}: AnalysesTypesDropDownProps) {



    const [lazyParams, setLazyParams] = useState(
        new AnalysesTypesModel().
            getStandardParam([{ field: 'business', order: 1 }, { field: 'company', order: 1 }, { field: 'point', order: 1 }, { field: 'type', order: 1 }], 
                AnalysesTypesService.defaultFilters()));
    ...
    useEffect(() => {
        if (initCatalog === false) {
            ...
            // Get Lazy Data
            AnalysesTypesService.getLazy(lazyEventSet).then((data: any) => {
